...
> 這一包要做的是 **第二大腦**：先給你一套方法，然後建一份「第二大腦自己的規矩」，接著我讀一份東西進去給你看、做一次整理檢查，最後用 Obsidian 打開它（沒裝的話我帶你裝，也可以之後再裝）。大概 60 分鐘，你要動手的主要是回答三個問題。
>
> 先講方法，講完我問你第一題。
...
> 好，接下來我們要做一件事：**幫你建一個會越用越懂你的知識庫**——你丟東西給我，我不是讀完就忘，而是整理成一頁一頁會互相連起來、以後直接查得到的筆記。
>
> 這套做法有個名字叫 **LLM Wiki**，是 Andrej Karpathy（OpenAI 創始成員、Tesla 前 AI 總監）提出來的，但你不用記這個名字，記住剛才那句話就好。
...
> - **放進來** — 你丟一份東西，我讀完，更新相關的筆記，建立彼此的連結
> - **問問題** — 你問，我從整理過的知識回答，附上出處。**好的答案我會存回知識庫**
> - **定期健檢** — 我檢查有沒有互相矛盾、過期的說法、沒人連到的孤兒筆記
...
> 好，我現在去把那份東西抓下來，放進 `raw/`——那是原料區，我只讀不改，原始檔案永遠留在那裡。
...
>
> 有哪一句聽起來跟你自己的理解不一樣，跟我說一聲；沒有的話我就當作對上了。
...
裝好之後，在 `core-rules.md` 的「我接上的外部服務（MCP）」表加一列 `obsidian｜在第二大腦裡搜尋、整理連結｜不用登入`（表放在 `<!-- tt:mcp START -->`…`<!-- tt:mcp END -->` 標記裡；還沒有就建，第 07 包也會往這張表加）。指令跑不動就把設定印出來請他貼上（Claude Code 貼進 `~/.claude.json` 的 `mcpServers`；Codex 貼在 `~/.codex/config.toml` 最後面，Windows 路徑一律寫成 `/`）：
...
**寫之前先 `Read`**：既有的 `knowledge/CLAUDE.md` 若**超過一行**，代表你這裡本來就放了一份完整的規矩（舊版本這樣設計）。**這種情況要先講一句人話再動手**：

> 你這裡本來就有一份規矩，我把它搬到 `AGENTS.md` 保留著、一個字都不改，這裡改放一行指過去就好——這樣兩個平台以後都讀得到同一份。

**先把它改名成 `knowledge/AGENTS.md`**（`AGENTS.md` 已存在就把內容併進去，不要覆蓋），再寫下面這一行入口。**不要直接一行蓋掉，那會把他原本寫的東西刪掉。**
...
每個月做一次，或是他說「整理第二大腦」的時候做（他說「tt 幫我健檢」是另一件事——那是整個家的健檢包）。查六項：
...
> 寫好了。以後你只要跟我說「**整理第二大腦**」，我就會照這六項檢查一遍。
>
> 或者你什麼都不用說——**每個月我自己會提**。
...
> 5. **一句「整理第二大腦」我會做的事**：照六項跑一遍，查完只報告，不自己亂動
>
...
**Q：健檢多久跑一次？**
每個月一次，或你說「整理第二大腦」。頁數少的時候幾秒就跑完，養成習慣比跑得勤重要。
